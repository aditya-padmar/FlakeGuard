  ...
  const startLive = useCallback(async (url: string, branch: string) => {
    ...
    try {
      ...
      if (result.status === 'no_tests') {
        setError('No pytest tests were found in this repository. FlakeGuard requires a Python project with pytest-compatible tests (files named test_*.py or *_test.py).');
        return;
      }
